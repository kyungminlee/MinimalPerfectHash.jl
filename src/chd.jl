...
function Base.iterate(c ::CHD{K, V}, count::Int=1) ::Union{Nothing, Tuple{Pair{K, V}, Int}} where {K, V}
  # skip empty slots
...
function Base.in(k, keys ::Base.KeySet{K, CHD{K,V}}) where {K, V}
  ti = chd_keyindex(keys.dict, K(k))
  return ti > 0
end

function Base.get(h::CHD{K,V}, key, default) where {K, V}
    index = chd_keyindex(h, key)
    @inbounds return (index <= 0) ? default : h.vals[index]::V
end

function Base.getkey(h::CHD{K,V}, key, default) where {K, V}
    index = chd_keyindex(h, key)
