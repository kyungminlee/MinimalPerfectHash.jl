...
"""
    CHD{K, V}(iter; max_collision=16777216, gamma=1.25, check_duplicates=true)

`CHD(iter)` construct a minmal perfect hash dictionary `CHD` of the values
generated by the given iterable object.
"""
function CHD{K, V}(
    kv;
    ...
    while !isempty(buckets)
        ...
        error(
            "Failed to find a collision-free hash function after $(max_collisions) attempts, " *
